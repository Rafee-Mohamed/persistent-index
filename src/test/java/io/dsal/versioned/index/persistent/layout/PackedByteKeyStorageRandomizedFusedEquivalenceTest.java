...
class PackedByteKeyStorageRandomizedFusedEquivalenceTest {
    ...
    /**
     * Oracle: {@code remove(removeIdx).insert(insertIdx, key)} — {@code insertIdx} is an index
     * into the sequence <em>after</em> {@code remove} (size {@code n - 1}), so {@code insertIdx}
     * in {@code [0, n - 1]} inclusive per {@link KeyStorage#insert(int, Object)}.
     */
    @Test
